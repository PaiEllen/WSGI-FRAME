...
from manager import session_type
...
class SessionFactory:
    ...
    def get_session_obj(handler):
        obj = None
        if session_type.SESSION_TYPE == "cache":
            obj = CacheSession(handler)
        elif session_type.SESSION_TYPE == "memcached":
            obj = MemcachedSession(handler)
        elif session_type.SESSION_TYPE == "redis":
...
class CacheSession:
    ...
    def __init__(self, handler):
        ...
        expires_time = time.time() + session_type.SESSION_EXPIRES
...
class MemcachedSession:
    ...
    def __init__(self, handler):
        ...
        if client_random_str and conn.get(client_random_str):
            ...
            self.random_str = create_session_id()
            conn.set(self.random_str, json.dumps({}), session_type.SESSION_EXPIRES)

        conn.set(self.random_str, conn.get(self.random_str), session_type.SESSION_EXPIRES)
    ...
    def __setitem__(self, key, value):
        ...
        ret_dict = json.loads(ret)
        ret_dict[key] = value
        conn.set(self.random_str, json.dumps(ret_dict), session_type.SESSION_EXPIRES)
...
class RedisSession:
    ...
    def __init__(self, handler):
        ...
        r.expire(self.random_str, session_type.SESSION_EXPIRES)
        expires_time = time.time() + session_type.SESSION_EXPIRES
